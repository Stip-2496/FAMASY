                    ...
                    $query->where(function ($q) {
                        $q->where('desHisMed', 'like', '%' . $this->search . '%')
                            ...
                            ->orWhereHas('animal', function ($animalQuery) {
                                $animalQuery->where('espAni', 'like', '%' . $this->search . '%')
                ...
                <div class="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between">
                    <div class="flex items-center space-x-2 mb-3 lg:mb-0">
                        ...
                        <div>
                            ...
                            <p class="text-gray-600 text-xs">Gestión de registros médicos de los animales</p>
                        </div>
                    </div>
                    
                    <div class="flex items-center space-x-2">
                        <a href="{{ route('pecuario.salud-peso.create') }}" wire:navigate
                           class="group relative inline-flex items-center px-4 py-2 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-300 overflow-hidden">
            ...
            <div class="flex flex-col sm:flex-row gap-2">
                ...
                <div class="flex-1 relative">
                    ...
                    <input type="text" 
                           ...
                           placeholder="Buscar descripción, responsable, ID animal..."
                ...
                <div class="relative">
                    ...
                    <select wire:model.live="tipo"
                            class="w-full pl-2 pr-8 py-1.5 text-xs bg-white border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-xs">
                        <thead class="bg-black">
                            <tr>
                                ...
                                <th class="px-2 py-1.5 text-left text-xs font-medium text-white uppercase tracking-wider whitespace-nowrap">Responsable</th>
                            ...
                            @foreach($historiales as $index => $historial)
                                <tr class="hover:bg-gray-50/50 transition-colors duration-200">
                                    <td class="px-2 py-1.5 whitespace-nowrap text-xs font-medium text-gray-900">
                                        {{ ($historiales->currentPage() - 1) * $historiales->perPage() + $index + 1 }}
                                    </td>
                                    <td class="px-2 py-1.5 whitespace-nowrap">
                                        @if($historial->animal)
                                            <div class="flex items-center gap-1">
                                                <div class="min-w-0">
                                                    <p class="text-xs font-medium text-gray-900">{{ $historial->animal->nitAni }}</p>
                                                </div>
                                            </div>
                                        @else
                                    ...
                                    <td class="px-2 py-1.5 whitespace-nowrap">
                                        @if($historial->animal)
                                            <div class="flex items-center gap-1">
                                                <div class="min-w-0">
                                                    <p class="text-xs text-gray-600">{{ ucfirst($historial->animal->espAni ?? 'N/A') }}</p>
                                                </div>
                                            </div>
                                        @else
                                            <span class="text-xs text-gray-600 italic">No asignado</span>
                                    ...
                                    <td class="px-2 py-1.5 whitespace-nowrap">
                                        @if($historial->animal)
                                            <div class="flex items-center gap-1">
                                                <div class="min-w-0">
                                                    <p class="text-xs text-gray-600">{{ ucfirst($historial->animal->razAni ?? 'N/A') }}</p>
                                                </div>
                                            </div>
                                        @else
                                            <span class="text-xs text-gray-600 italic">No asignado</span>
                                    ...
                                    <td class="px-2 py-1.5 whitespace-nowrap">
                                        @if($historial->tipHisMed == 'vacuna')
                                            <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-xs font-medium bg-green-100 text-green-600">
                                                Vacuna
                                            </span>
                                        @elseif($historial->tipHisMed == 'tratamiento')
                                            <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-xs font-medium bg-yellow-100 text-yellow-600">
                                                Tratamiento
                                            </span>
                                        @else
                                    ...
                                    <td class="px-2 py-1.5 whitespace-nowrap">
                                        <div class="flex items-center gap-1">
                                            ...
                                            <div class="min-w-0">
                                                <p class="text-xs text-gray-900">{{ \Carbon\Carbon::parse($historial->fecHisMed)->format('d/m/Y') }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-2 py-1.5 whitespace-nowrap text-xs text-gray-700">
                            ...
                            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                                <div>
                                    <p class="text-xs text-gray-700">
                                        Mostrando <span class="font-medium">{{ $historiales->firstItem() }}</span> a 
                                        <span class="font-medium">{{ $historiales->lastItem() }}</span> de 
                                        <span class="font-medium">{{ $historiales->total() }}</span> resultados
                                    </p>
                                </div>
                                <div>
                                    {{ $historiales->links() }}
                ...
                <div class="text-center py-4 px-4">
                    <div class="w-8 h-8 bg-gray-100 rounded-full mx-auto mb-2 flex items-center justify-center">
                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5h6m-3-3v6m-6 3h12m-3 3h-6m-3-3v6m6-3v6"></path>
                        </svg>
                    </div>
                    <h3 class="text-sm font-medium text-gray-900 mb-1">No hay registros médicos</h3>
                    <p class="text-xs text-gray-600 mb-2">
                        @if($tipo || $fecha || $search)
                            No hay resultados para los filtros aplicados. Intenta con otros términos.
                        @else
                            No hay registros médicos en el sistema.
                        @endif
                    </p>
            ...
            <div class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50" wire:click.self="$set('showDeleteModal', false)">
                <div class="bg-white rounded-2xl shadow-xl max-w-md w-full p-5 transform transition-all duration-300">
                    <div class="text-center">
                        ...
                        <p class="text-xs text-gray-600 mb-3">¿Estás seguro de que deseas eliminar este registro médico? Esta acción no se puede deshacer.</p>
                        
                        @php
                            $historial = HistorialMedico::with('animal')->find($historialToDelete);
                        @endphp
                            ...
                            <div class="bg-gray-50 rounded-lg p-2.5 mb-3 text-left">
                                <div class="space-y-1 text-xs">
                                    <div class="flex justify-between">
                                        ...
                                        <span class="text-gray-900">{{ ucfirst($historial->tipHisMed) }}</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="font-medium text-gray-700">Animal:</span>
                                        <span class="text-gray-900">{{ $historial->animal->nitAni ?? 'Animal #'.$historial->idAniHis }}</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="font-medium text-gray-700">Fecha:</span>
                                        <span class="text-gray-900">{{ \Carbon\Carbon::parse($historial->fecHisMed)->format('d/m/Y') }}</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="font-medium text-gray-700">Responsable:</span>
                                        <span class="text-gray-900">{{ $historial->responHisMed }}</span>
                                    </div>
                                </div>
                            </div>
                        @endif
