...
new #[Layout('layouts.auth')] class extends Component {
    ...
    public function mount(ProduccionAnimal $produccion)
    {
        ...
        $this->animales = Animal::where('estAni', 'vivo')->get(['idAni', 'nitAni', 'espAni', 'razAni', 'sexAni']);
    ...
    private function cargarDatosProduccion()
    {
        $this->idAniPro = $this->produccion->idAniPro;
        $this->tipProAni = $this->produccion->tipProAni;
        $this->canProAni = $this->produccion->canProAni;
        $this->uniProAni = $this->produccion->uniProAni;
        $this->fecProAni = $this->produccion->fecProAni ? $this->produccion->fecProAni->format('Y-m-d') : '';
        $this->obsProAni = $this->produccion->obsProAni;
        $this->canTotProAni = $this->produccion->canTotProAni;

        // Configurar selecciones basadas en el animal existente
        if ($this->idAniPro) {
            $this->animalSeleccionado = Animal::find($this->idAniPro);
            if ($this->animalSeleccionado) {
                $this->especieSeleccionada = $this->animalSeleccionado->espAni;
                $this->configurarOpcionesProduccion($this->especieSeleccionada);
                ...
                $this->razaSeleccionada = $this->animalSeleccionado->razAni;
                ...
                // Configurar categoría y subcategoría basadas en el tipo de producción existente
                $this->configurarCategoriaDesdeTipoProduccion();
            }
        }
    }
    ...
    public function updatedCategoriaProduccion($value)
    {
        ...
        if (isset($this->opcionesPorEspecie[$especieKey])) {
            ...
            // Actualizar el tipo de producción
            $this->actualizarTipoProduccion();
        }
    }

    public function updatedSubcategoriaProduccion($value)
    {
        $this->actualizarTipoProduccion();
    }
    ...
    private function configurarCategoriaDesdeTipoProduccion()
    {
        $tipoProduccion = $this->tipProAni;
        
        // Mapeo inverso de tipos de producción a categorías
        $mapeoInverso = [
            'leche bovina' => 'leche_bovina',
            'venta en pie bovino' => 'venta_pie_bovino',
            'leche ovina' => 'leche_ovina',
            'lana ovina' => 'lana_ovina',
            'venta en pie ovino' => 'venta_pie_ovino',
            'venta gallinas en pie' => 'venta_gallinas_pie',
            'venta pollo engorde' => 'venta_pollo_engorde',
            'huevo A' => 'huevos',
            'huevo AA' => 'huevos',
            'huevo AAA' => 'huevos',
            'huevo Jumbo' => 'huevos',
        ...
        $this->categoriaProduccion = $mapeoInverso[$tipoProduccion] ?? 'otros';
        
        // Configurar subcategoría si es necesario
        if (in_array($tipoProduccion, ['huevo A', 'huevo AA', 'huevo AAA', 'huevo Jumbo', 'huevo B', 'huevo C'])) {
            $this->subcategoriaProduccion = $tipoProduccion;
            $this->mostrarSubcategoria = true;
            
            $especieLower = strtolower($this->especieSeleccionada);
            $especieKey = $this->determinarEspecieKey($especieLower);
            if (isset($this->opcionesPorEspecie[$especieKey])) {
                $this->opcionesSubcategoria = $this->opcionesPorEspecie[$especieKey]['subcategorias'][$this->categoriaProduccion] ?? [];
            }
        }
        
        // Configurar unidad de medida
        $this->configurarUnidadDesdeCategoria();
    }

    private function configurarUnidadDesdeCategoria()
    {
        if (!$this->especieSeleccionada || !$this->categoriaProduccion) return;
        
        $especieLower = strtolower($this->especieSeleccionada);
        $especieKey = $this->determinarEspecieKey($especieLower);
        
        if (isset($this->opcionesPorEspecie[$especieKey])) {
            $this->uniProAni = $this->opcionesPorEspecie[$especieKey]['unidades'][$this->categoriaProduccion] ?? '';
        }
    }

    private function actualizarTipoProduccion()
    {
        // Si hay subcategoría específica (como tipos de huevos), usar esa
        if ($this->mostrarSubcategoria && $this->subcategoriaProduccion) {
            $this->tipProAni = $this->subcategoriaProduccion;
            return;
        }
        ...
        $mapeoTipos = [
            ...
            'venta_gallinas_pie' => 'venta gallinas en pie',
            'venta_pollo_engorde' => 'venta pollo engorde',
            'huevos' => 'huevo A', // Valor por defecto para huevos
            'otros' => 'otros'
        ];

        $this->tipProAni = $mapeoTipos[$this->categoriaProduccion] ?? 'otros';
    }
    ...
    {
        // Asegurarse de que el tipo de producción esté actualizado
        $this->actualizarTipoProduccion();
    ...
    public function debug()
    {
        ...
        $tipoProduccionFinal = $this->tipProAni;
...
<div class="flex items-center justify-center min-h-screen py-3">
    <div class="w-full max-w-7xl mx-auto bg-white/80 backdrop-blur-xl shadow rounded-3xl p-3 relative border border-white/20">
        ...
        <div class="absolute top-2 right-2">
            <a href="{{ route('pecuario.produccion.index', $produccion->idProAni) }}" wire:navigate
        ...
        <div class="text-center mb-3"
            ...
            <template x-if="!showSuccess && !showCancel">
                <p class="text-gray-600 text-xs">Modifique los datos de la producción existente</p>
            </template>
        </div>
                ...
                <div class="p-2">
                    <div class="flex items-center space-x-2 mb-2">
                        ...
                        <div>
                            ...
                            <p class="text-gray-600 text-[10px]">Modifique el animal asociado a la producción</p>
                        </div>
                    </div>
                    ...
                    <div class="p-2">
                        <div class="flex items-center space-x-2 mb-2">
                            ...
                            <div>
                                ...
                                <p class="text-gray-600 text-[10px]">Modifique la categoría y tipo de producción</p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div>
                                ...
                                <div class="relative group">
                                    <select 
                                        wire:model.live="subcategoriaProduccion" 
                    ...
                    <div class="p-2">
                        <div class="flex items-center space-x-2 mb-2">
                            ...
                            <div>
                                ...
                                <p class="text-gray-600 text-[10px]">Modifique la información cuantitativa</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                            ...
                            <div>
                                ...
                                <p class="text-gray-500 text-[10px] mt-0.5 flex items-center">
