...
new #[Layout('layouts.auth')] class extends Component {
        ...
        try {
            switch ($this->periodo) {
                ...
                case 'semestre':
                    ...
                    $this->fechaFin = now()->addMonths(5)->endOfMonth()->format('Y-m-d');
                    break;
                ...
                case 'personalizado':
                    // No cambiar fechas, esperar a que el usuario las configure
                    return;
            }
            
            // Actualizar fechas personalizadas para mantener consistencia
            $this->fecha_personalizada_desde = $this->fechaInicio;
            $this->fecha_personalizada_hasta = $this->fechaFin;
            
            // Recargar reportes con el nuevo período
            $this->cargarReportesCompletos();
            
            // Emitir evento para actualizar gráficos
            $this->dispatch('periodo-actualizado');
            
        } catch (\Exception $e) {
            Log::error('Error actualizando período: ' . $e->getMessage());
            session()->flash('error', 'Error al actualizar el período');
        }
    }
     ...
     public function aplicarPeriodoPersonalizado()
    {
        try {
            ...
            if (empty($this->fecha_personalizada_desde) || empty($this->fecha_personalizada_hasta)) {
            ...
            if ($this->fecha_personalizada_desde > $this->fecha_personalizada_hasta) {
                session()->flash('error', 'La fecha de inicio no puede ser mayor que la fecha de fin');
                return;
            }
            
            // Validar que no sea un rango muy extenso (más de 2 años)
            $inicio = Carbon::parse($this->fecha_personalizada_desde);
            $fin = Carbon::parse($this->fecha_personalizada_hasta);
            
            if ($inicio->diffInDays($fin) > 730) {
                session()->flash('error', 'El rango de fechas no puede ser mayor a 2 años');
                return;
            }
            ...
            $this->registrarAccionAuditoria('PERIODO_PERSONALIZADO', 'Período personalizado aplicado', [
            ...
            Log::error('Error aplicando período personalizado: ' . $e->getMessage());
            session()->flash('error', 'Error al aplicar el período personalizado');
        }
    }
    ...
    public function actualizarReportes()
    {
        try {
            ...
            Log::error('Error actualizando reportes: ' . $e->getMessage());
            session()->flash('error', 'Error al actualizar los reportes');
        }
    }
    ...
    public function actualizarDashboard()
    {
        ...
        // Disparar evento para reinicializar gráficos
...
<div>
    ...
    console.log('📊 Datos cargados:', { datosFlujoCaja, datosCategorias });
    ...
    document.addEventListener('livewire:init', () => {
        Livewire.on('periodo-cambiado', (data) => {
            console.log('📅 Período cambiado:', data);
            graficosInicializados = false;
            setTimeout(actualizarGraficos, 500);
        });

        Livewire.on('dashboard-actualizado', () => {
